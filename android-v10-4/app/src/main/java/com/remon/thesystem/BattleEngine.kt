...
class BattleEngine(private val store: GameStore) {
    ...
    private var enemyMaxHpInternal = goblin.hp
    ...
    fun start(rank: String) {
        ...
        logs.clear()
        spawnForRoom()
        logs.add("${enemy.name} appeared. Read the intent before acting.")
        startRound()
    }
    ...
    private fun spawnForRoom() {
        ...
        enemyMaxHpInternal = (enemy.hp * scale).roundToInt()
        enemyHp = enemyMaxHpInternal
    ...
    private fun chooseIntent(): Intent {
        ...
        val enraged = enemyHp < enemyMaxHpInternal/2
        return if(enraged && round%2==0) Intent("Berserker Rush",18,22,"MULTI",28,false,false,2)
        else Intent("Ground Slam",34,42,"BOSS",25)
    }

    fun snapshot() = BattleSnapshot(enemy.name,enemyHp,enemyMaxHpInternal,enemyBreak,enemy.breakMax,playerHp,playerMp,ap,round,intent,analyzed,brokenTurns>0,bleedTurns,logs.toList(),finished,victory)
    ...
    fun vitalStrike(timing: String): String {
        ...
        ap-=2;playerMp-=12;vitalCooldown=3
    ...
    fun guard(timing: String) {
        if(!spendAp(1))return
        guarded=true;perfectGuard=timing=="PERFECT";guardShield=if(perfectGuard)80 else if(timing=="MISS")20 else 50
        logs.add(0,when{perfectGuard->"PERFECT GUARD prepared.";timing=="MISS"->"Late Guard: only 20% reduction.";else->"Guard prepared."})
        autoEnemyTurnIfNeeded()
    }
    ...
    private fun damageEnemy(damage:Int,breakDmg:Int,label:String){
        ...
        if(enemyBreak==0&&brokenTurns==0){brokenTurns=2;logs.add(0,"BREAK — ${enemy.name} loses its action and takes +25% damage on your next turn.")}
    ...
    private fun enemyTurn(){
        ...
        if(perfectGuard){enemyBreak=(enemyBreak-8).coerceAtLeast(0);logs.add(0,"Perfect Guard counter dealt 8 Break.");if(enemyBreak==0&&brokenTurns==0){brokenTurns=2;logs.add(0,"BREAK from Perfect Guard!")}}
        store.save()
    ...
    private fun nextRound(){
        ...
        if(brokenTurns==1){
            startRound()
        } else if(brokenTurns==0 && enemyBreak==0) {
            enemyBreak=enemy.breakMax
            startRound()
        } else startRound()
    }
